...
export default function AIAssistantPage() {
  ...
  return (
    ...
    <Sheet open={mobileConvsOpen} onOpenChange={setMobileConvsOpen}>
      <SheetContent side="left" className="w-80 p-0 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          <Button size="sm" onClick={() => { startNewConversation(); setMobileConvsOpen(false) }} className="w-full cursor-pointer">
          ...
          {loadingConversations ? (
            ...
            <div className="space-y-1">
              {conversations.map((conv) => (
                <div
                  key={conv.id}
                  onClick={() => { loadConversation(conv.id); setMobileConvsOpen(false) }}
            ...
            <div className="space-y-3">
              {materiaOptions.map((materia) => (
                <div key={materia} className="flex items-center justify-between space-x-2">
                  <Label htmlFor={`m-mobile-${materia}`} className="text-sm font-normal cursor-pointer flex-1">
          ...
          <div className="space-y-3 p-4">
            {sources.map((source, i) => (
              <Card
                ...
                onClick={() => { handleTesisClick(source.id_tesis); setMobileSourcesOpen(false) }}
              >
                <CardContent className="p-4">
                  ...
                  <h4 className="font-medium text-sm mb-2 line-clamp-2">{source.rubro}</h4>
                  <div className="flex items-center gap-2 text-xs text-muted-foreground flex-wrap">
                    <Badge variant="outline" className="text-xs">{source.tipo_tesis}</Badge>
    ...
    <div className="h-full p-4 pt-16 md:p-6 md:pt-6">
      <div className="flex gap-4 h-full">
        {/* Sidebar - Conversations (desktop only) */}
        <div className="hidden md:flex w-80 flex-col gap-4 h-full overflow-hidden">
        {/* Conversations */}
          ...
          <CardHeader className="border-b">
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                onClick={() => setMobileConvsOpen(true)}
      ...
      {sources.length > 0 && (
        <div className={`hidden md:flex w-96 h-full overflow-hidden ${sourcesAnimating ? 'animate-in fade-in slide-in-from-right-4 duration-400' : ''}`}>
