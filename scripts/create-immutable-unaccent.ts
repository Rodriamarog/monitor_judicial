...
async function main() {
  console.log('Creating immutable unaccent wrapper and indexes...\n')
  ...
  try {
    ...
    // Step 2: Create immutable wrapper function
    console.log('Step 2: Creating immutable unaccent wrapper...')
    await pool.query(`
      ...
      $func$
    `)
    console.log('✅ Immutable unaccent wrapper created\n')

    // Step 3: Create index on rubro
    console.log('Step 3: Creating index on rubro (this may take a few minutes)...')
    const startRubro = Date.now()
    await pool.query(`
      CREATE INDEX CONCURRENTLY IF NOT EXISTS tesis_documents_rubro_unaccent_idx
      ON tesis_documents
      USING gin (immutable_unaccent(rubro) gin_trgm_ops)
    `)
    const rubroTime = Math.floor((Date.now() - startRubro) / 1000)
    console.log(`✅ Rubro index created in ${rubroTime}s\n`)

    // Step 4: Create index on texto
    console.log('Step 4: Creating index on texto (this may take 5-10 minutes)...')
    const startTexto = Date.now()
    await pool.query(`
      CREATE INDEX CONCURRENTLY IF NOT EXISTS tesis_documents_texto_unaccent_idx
      ON tesis_documents
      USING gin (immutable_unaccent(texto) gin_trgm_ops)
    `)
    ...
    console.log('\nNext step: Update search query to use immutable_unaccent() instead of unaccent()')
