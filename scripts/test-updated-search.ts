...
async function main() {
  console.log('Testing updated search query (without unaccent)...\n')

  const searchQuery = 'facturas electronicas'

  try {
    // Test the updated query (without unaccent)
    ...
    // Now actually run the query and time it
    console.log('\n\n=== Actual Query Test ===\n')
    ...
    console.log(`\n${duration < 1000 ? '✅' : '⚠️'} Query completed in ${duration}ms`)
    console.log(`${duration < 1000 ? 'MUCH FASTER!' : 'Still slow, needs investigation'}`)
